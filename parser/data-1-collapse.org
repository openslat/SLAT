...
* Total Loss
  ...
  #+BEGIN_SRC R   :results output
    paleo.structloss.c <- read.csv(
        ...
        skip=3, header=FALSE, sep="")
    names(paleo.structloss.c) = c("IM", "mean_ln_x", "sd_ln_x")

    paleo.structloss.nc <- read.csv(
        paste(results.dir, "tlossnc", sep=""),
        skip=3, header=FALSE, sep="")
    names(paleo.structloss.nc) = c("IM", "mean_ln_x", "sd_ln_x")
...
* Non-Collapse
  #+BEGIN_SRC R 
    diff <- paleo.structloss.nc$mean_ln_x -
        neo.structloss.nc$mean_ln_x
    denom <- paleo.structloss.nc$mean_ln_x
    denom[denom == 0] <- NA
    diff.pct <- diff / denom
  ...
  #+BEGIN_SRC R
    palette(c("red", "blue"))
    plot(paleo.structloss.nc$mean_ln_x ~
         paleo.structloss.nc$IM, log="xy", 
         xlab="IM", ylab="Mean(ln(Loss))", 
         main="Total Loss, No Collapse",
         type="p", col=1, lwd=5)

    lines(neo.structloss.nc$mean_ln_x ~ neo.structloss.nc$IM, 
        col=2, lwd=3)
...
* Collapse
  #+BEGIN_SRC R 
    diff <- paleo.structloss.c$mean_ln_x - 
        neo.structloss.c$mean_ln_x
    denom <- paleo.structloss.c$mean_ln_x
    denom[denom == 0] <- NA
    diff.pct <- diff / denom
  ...
  #+BEGIN_SRC R
    palette(c("red", "blue"))
    plot(paleo.structloss.c$mean_ln_x ~ paleo.structloss.c$IM, 
         log="xy", xlab="IM", ylab="Mean(ln(Loss))", 
         main="Total Loss, Collapse",
         type="p", col=1, lwd=5)

    lines(neo.structloss.c$mean_ln_x ~ neo.structloss.c$IM, 
        col=2, lwd=3)
  ...
  [[file:/tmp/babel-1752Epe/figure-1752lH2.pdf]]
  ...
  [[file:/tmp/babel-1752Epe/figure-1752xlR.pdf]]
