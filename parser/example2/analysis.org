...
* Initialisation
  ...
  #+BEGIN_SRC R  :results output 
    ...
    neo.im <- read.csv("py-results/im_rate.csv", 
                       header=TRUE, sep="")
    ...
    for (i in seq(num.edps)) {
        ...
        neo.edp.im[[i]] <- read.csv(
            sprintf("py-results/im_edp_%d.csv", i),
            header=TRUE, sep="")
    ...
    for (i in seq(num.edps)) {
        ...
        neo.edp.rate[[i]] <- read.csv(
            sprintf("py-results/edp_%d_rate.csv", i),
            header=TRUE, sep="")
    ...
    neo.collapse.im <- read.csv("py-results/collapse.csv", 
                                header=TRUE, sep="")
    ...
    for (i in seq(num.cgs)) {
        ...
        neo.loss.im[[i]] <- read.csv(
            sprintf("py-results/cost_%d_im.csv", i),
            header=TRUE, sep="")
        ...
        neo.loss.edp[[i]] <- read.csv(
            sprintf("py-results/cost_%d_edp.csv", i),
            header=TRUE, sep="")
...
* COLLAPSE
  ...
  #+BEGIN_SRC R  :results value 
    paleo.rate <- scan(paste(results.dir, "collrate.txt", 
                             sep=""), skip=3)
    neo.rate <- as.numeric(scan("py-results/collrate.csv", 
                                what="string")[8])
    paste(
...
** Loss by Fate (Lines)
    ...
    #+BEGIN_SRC R :session *R-2*
      loss.by.fate <- read.csv("py-results/cost_by_fate", header=TRUE, sep="")
...
** Loss by Fate (Solid)
    ...
    #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
    #+BEGIN_SRC R :session *R-2*
      loss.by.fate.summed <- read.csv("py-results/cost_by_fate", header=TRUE, sep="")
...
** Loss Total, Normalised
    *I'm not sure these calculations are correct*
    ...
    #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
    #+BEGIN_SRC R :session *R-2*
      loss.by.fate.normalised <- read.csv("py-results/cost_by_fate", header=TRUE, sep="")
      ...
      im.linear <- read.csv("py-results/im_rate_lin", header=TRUE, sep="")
...
* Loss by EDP
   ...
   #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
   #+BEGIN_SRC R :session *R-2*
     loss.by.edp <- read.csv("py-results/cost_by_edp", header=TRUE, sep="")
...
* Loss by Component Type
   ...
   #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
   #+BEGIN_SRC R :session *R-2*
     loss.by.comp <- read.csv("py-results/cost_by_frag", 
                              header=TRUE, sep="")
   ...
   #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
   #+BEGIN_SRC R :session *R-2*
     loss.by.comp <- read.csv("py-results/cost_by_frag", header=TRUE, sep="")
     palette(rainbow(length(comp.names)))
...
#+end_example
   ...
   #+BEGIN_SRC sh :results output
   for i in $(seq 21); do    
      f=example1a_edp_${i}_rate.csv
      min_n=$(head -2 $f | tail -1 | awk -e '{print $1}')
      max_n=$(tail -1 $f | awk -e '{print $1}')
      count_n=$(wc -l $f | awk -e '{print $1 - 1}')

      f=~/SLATv1.15_Public/example1a_collapse/results/edp-${i}-rate.csv
      min_p=$(head -4 $f | tail -1 | awk -e '{print $1}')
      max_p=$(tail -2 $f | awk -e 'BEGIN{RS="\r\n"} {print $1}')
      count_p=$(wc -l $f | awk -e '{print $1 - 4}')
      echo $min_n $min_p "; " $max_n $max_p "; " $count_n $count_p
   ...
   #+BEGIN_SRC R :session *R-2*
     graph.ds <- function(n) {
         ...
         for (i in 1:length(cgs$Index)) {
             cg = cgs$Index[i]
             ds.im <- read.csv(
                 sprintf("py-results/ds_im_%d.csv", cg), header=TRUE, sep="")
