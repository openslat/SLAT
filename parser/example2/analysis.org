...
* Initialisation
  ...
  #+BEGIN_SRC R  :results output 
    ...
    neo.im <- read.csv("c-results/im_rate", 
                       header=TRUE, sep="")
    ...
    for (i in seq(num.edps)) {
        ...
        neo.edp.im[[i]] <- read.csv(
            sprintf("c-results/im_edp_%d.txt", i),
            header=TRUE, sep="")
    ...
    for (i in seq(num.edps)) {
        ...
        neo.edp.rate[[i]] <- read.csv(
            sprintf("c-results/edp_%d_rate.txt", i),
            header=TRUE, sep="")
    ...
    neo.collapse.im <- read.csv("c-results/collapse.txt", 
                                header=TRUE, sep="")
    ...
    for (i in seq(num.cgs)) {
        ...
        neo.loss.im[[i]] <- read.csv(
            sprintf("c-results/loss_%d_im.txt", i),
            header=TRUE, sep="")
        ...
        neo.loss.edp[[i]] <- read.csv(
            sprintf("c-results/loss_%d_edp.txt", i),
            header=TRUE, sep="")
    ...
    paleo.structloss.c <- read.csv(
        paste(results.dir, "loss_c_total", sep=""),
        skip=3, header=FALSE, sep="")
    ...
    if (length(paleo.structloss.nc$IM) !=
        length(neo.structloss.nc$IM) ||
        (max(abs((paleo.structloss.nc$IM - 
                      neo.structloss.nc$IM)/
    ...
    neo.structloss.c <- read.csv("c-results/loss_c_total", 
                                 header=TRUE, sep="")
    names(neo.structloss.c) = c("IM", "mean.x", "sd.lnx")

    neo.structloss.nc <- read.csv("c-results/loss_nc_total", 
                  		header=TRUE, sep="")
    names(neo.structloss.nc) = c("IM", "mean.x", "sd.lnx")


    if (length(paleo.structloss.c$IM) != 
        length(neo.structloss.c$IM) ||
    ...
    neo.deagg <- read.csv("c-results/deagg", header=TRUE, sep="")
    names(neo.deagg) <- c("IM", "mean.nc", "sd.nc", 
  			"mean.c", "sd.nc")
  ...
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/im_rate': No such file or directory
  Error in file(file, "rt") (from #3) : cannot open the connection
  In addition: Warning message:
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/im_edp_1.txt': No such file or directory
  Error in file(file, "rt") (from #3) : cannot open the connection
  In addition: Warning message:
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/edp_1_rate.txt': No such file or directory
  Error in file(file, "rt") : cannot open the connection
  In addition: Warning message:
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/collapse.txt': No such file or directory
  Error in names(neo.collapse.im) = c("IM", "pDemolition", "pCollapse") : 
    'names' attribute [3] must be the same length as the vector [2]
  Error in file(file, "rt") (from #2) : cannot open the connection
  In addition: Warning message:
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/loss_1_im.txt': No such file or directory
  Error in file(file, "rt") : cannot open the connection
  In addition: Warning message:
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/loss_c_total': No such file or directory
  Error in file(file, "rt") : cannot open the connection
  In addition: Warning message:
  In file(file, "rt") :
    cannot open file '/home/mag109/SLATv1.15_Public/example2_10storeybuilding/results/loss_nc_total': No such file or directory
#+end_example
...
* COLLAPSE
  ...
  #+BEGIN_SRC R  :results value 
    paleo.rate <- scan(paste(results.dir, "collrate.txt", 
                             sep=""), skip=3)
    neo.rate <- as.numeric(scan("c-results/collrate.txt", 
                                what="string")[8])
    paste(
...
* Loss by Fate
   ...
   #+BEGIN_SRC R :session *R-2*
     loss.by.fate <- read.csv("c-results/loss_by_fate", header=TRUE, sep="")
     #loss.by.fate["demo.mean"] = loss.by.fate["demo.mean"] + loss.by.fate["coll.mean"]
     #loss.by.fate["repair.mean"] = loss.by.fate["repair.mean"] + loss.by.fate["demo.mean"]
     ...
     for (i in 1:length(fate.names)) {
         lines(loss.by.fate[,(i+1)] ~ loss.by.fate[,"IM"], col=i)
     }
     legend(x="topleft", ncol=2, legend=c(fate.names), fill=palette())
   #+END_SRC
   ...
   [[file:/tmp/babel-1768wy-/figure-1768RdZ.pdf]]
...
* Loss by EDP
   ...
   #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
   #+BEGIN_SRC R :session *R-2*
     loss.by.edp <- read.csv("c-results/loss_by_edp", header=TRUE, sep="")
     ...
     plot(neo.structloss.nc$mean.x ~ neo.structloss.nc$IM)
     for (i in 1:length(edp.names)) {
         lines(loss.by.edp[,(i+1)] ~ loss.by.edp[,"IM"], col=i)
     }
   ...
   [[file:/tmp/babel-1768wy-/figure-17689vR.pdf]]
...
* Loss by Component Type
   ...
   #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
   #+BEGIN_SRC R :session *R-2*
     loss.by.comp <- read.csv("c-results/loss_by_frag", header=TRUE, sep="")
     comp.names = names(loss.by.comp)
     comp.names = comp.names[2:length(comp.names)]
     for (i in 2:length(comp.names)) {
         loss.by.comp[comp.names[i]] = loss.by.comp[comp.names[i]] + loss.by.comp[comp.names[i-1]]
     }
     palette(rainbow(length(comp.names)))
     plot(neo.structloss.nc$mean.x ~ neo.structloss.nc$IM)
     for (i in 1:length(comp.names)) {
         lines(loss.by.comp[,(i+1)] ~ loss.by.comp[,"IM"], col=i)
     }
     legend(x="topleft", ncol=2, legend=c(comp.names), fill=palette())
   #+END_SRC
   ...
   [[file:/tmp/babel-1768wy-/figure-1768xYq.pdf]]
...
** Collapse
   ...
   #+BEGIN_SRC R  
     palette(c("red", "blue"))
     plot(paleo.structloss.c$sd.lnx ~ paleo.structloss.c$IM, 
...
* Deaggregated Loss
   Plot the mean deaggregated loss (Figure [[fig:deagg.loss-mean]]):
   #+NAME: deagg.loss-mean
   #+HEADER: :results graphics
   #+HEADER: :file (org-babel-temp-file "./figure-" ".pdf")
   #+BEGIN_SRC R
     palette(c("red", "blue"))
     plot(neo.deagg$mean.nc ~ neo.deagg$IM,
          log="", xlab="IM", ylab="Mean(Loss) ($M)",
          xlim=c(0, 1.5),
          ylim=c(0, max(neo.deagg$mean.c, neo.deagg$mean.nc)),
          main="Total Loss, Collapse",
          type="l", col=1, lwd=5, yaxt="n")

     lines(neo.deagg$mean.nc + neo.deagg$mean.c ~ neo.deagg$IM, 
         col=2, lwd=3)

     axis(2, c(0:14)*1E6, c(0:14))

     legend(x="topleft",
          legend=c("No Collapse", "Collapse"),
          fill=palette())

     polygon(c(neo.deagg$IM, rev(neo.deagg$IM)), 
             c(neo.deagg$mean.nc, rep(0, length(neo.deagg$IM))), 
             col="red")
     polygon(c(neo.deagg$IM, rev(neo.deagg$IM)), 
             c(neo.deagg$mean.c + neo.deagg$mean.nc,
               rev(neo.deagg$mean.nc)), col="blue")
   #+END_SRC

   #+CAPTION: Mean of total loss, considering collapse.
   #+ATTR_LaTeX: :width \textwidth*4/4 :placement [h!bt]
   #+NAME: fig:deagg.loss-mean
   #+RESULTS: deagg.loss-mean
   [[file:/tmp/babel-17848lX/figure-1784TWh.pdf]]
...
* Annual Loss
  ...
  #+BEGIN_SRC R  :results value 
    paleo.ann.loss <- scan(paste(results.dir, "ann_loss", 
                             sep=""), skip=3)
    neo.ann.loss <- as.numeric(scan("c-results/ann_loss",
                                    skip=1))
  ...
  #+RESULTS:
  |              |     Paleo |       Neo | Error(%) |
  | E(loss)      | 1.175e+04 | 1.168e+04 |    0.584 |
  | SD(ln(loss)) | 2.491e+00 | 2.493e+00 |    0.084 |
